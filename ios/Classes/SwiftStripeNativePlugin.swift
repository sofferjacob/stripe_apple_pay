...
public class SwiftStripeNativePlugin: NSObject, FlutterPlugin, PKPaymentAuthorizationViewControllerDelegate {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        } else if (call.method == "presentApplePay") {
            guard var orderDict = call.arguments as? Dictionary<String, Any?> else {
                hand(error: StripeNativeError.PaymentParameterTypeMismatch(message: "The order used contains invalid arguments"))
                return
            }

            guard let merchantName = orderDict.removeValue(forKey: "merchantName") as? String else {
                hand(error: StripeNativeError.MissingMerchantName(message: "setMerchantName must be called before using presentApplePay."))
                return
            }

            guard let subtotal = orderDict.removeValue(forKey: "subtotal") as? Double else {
                hand(error: StripeNativeError.MissingMerchantName(message: "Invalid Order: subtotal missing"))
                return
            }
            guard let tax = orderDict.removeValue(forKey: "tax") as? Double else {
                hand(error: StripeNativeError.MissingMerchantName(message: "Invalid Order: tax missing"))
                return
            }
            guard let tip = orderDict.removeValue(forKey: "tip") as? Double else {
                hand(error: StripeNativeError.MissingMerchantName(message: "Invalid Order: tip missing"))
                return
            }

            let items = orderDict.compactMap { (arg) -> Item in
                let (key, val) = arg
                guard let price = val as? Double else {
                    hand(error: StripeNativeError.PaymentParameterTypeMismatch(message: "An item in the Order object contains a price that is not a number."))
                    return Item(name: key, price: 0)
                }
                return Item(name: key, price: price)
            }

            fetchStripeToken(result: result, items: items, name: merchantName, subtotal: subtotal, tip: tip, tax: tax)
    ...
    private func fetchStripeToken(result: @escaping FlutterResult, items: [Item], name: String, subtotal: Double, tip: Double, tax: Double) {
        guard let identifier = merchantIdentifier else {
            hand(error: StripeNativeError.MissingMerchantIdentifier(message: "setMerchantIdentifier must be called before using presentApplePay"))
            return
        }
        ...
        var total = items.reduce(0) { (next, item) -> Double in 
        ...
        if (subtotal != -1) {
            paymentRequest.paymentSummaryItems.append(PKPaymentSummaryItem(label: "subtotal", amount: NSDecimalNumber(floatLiteral: subtotal)))
        }
        if (tip != -1) {
            paymentRequest.paymentSummaryItems.append(PKPaymentSummaryItem(label: "tip", amount: NSDecimalNumber(floatLiteral: tip)))
            total += tip
        }
        if (tax != -1) {
            paymentRequest.paymentSummaryItems.append(PKPaymentSummaryItem(label: "tax", amount: NSDecimalNumber(floatLiteral: tax)))
            total += tax
        }
